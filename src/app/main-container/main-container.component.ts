...
export class MainContainerComponent implements OnInit {
  ...
  selector:string = "Scrapping";// pour afficher la page correspondate

  strToAnalyze:string;//String to be analyze
  sentimentState:boolean;//To show sentiment analyze of phrase

  nDeaths:number=0;
  nCured:number=0;
  nCases:number=0;

  nToPredict:number=100;//Number of cases to Predict
  nOfDeathsPredicted:number=70;//Number predicted of deaths
  ...
  predictNews(){
    console.log(this.nToPredict);
    this.nOfDeathsPredicted = this.nToPredict/2;
    this.ChartsPrediction();
  }

  AnalyzePhrase(){
    this.serT.AnalyzePhrase(this.strToAnalyze).subscribe( (data:string) => {
      if(data == "p"){
        this.sentimentState=true;
      }else{
        this.sentimentState=false;
      }
    });
  }
  ...
  CalculCases(){
    ...
    this.nCured=0;
    this.nCases=0;
    this.dataMaroc.Deaths.forEach(element => {
      this.nDeaths = this.nDeaths + element;
    });
    this.dataMaroc.Confirmed.forEach(element => {
      this.nCases = this.nCases+ element;
    });
    this.dataMaroc.Recovered.forEach(element => {
  ...
  ChartsPrediction(){
    var predictChart = new Chart("predictChart", {
      ...
      data: {
        labels: ["Deaths" ,"Cases"],
        datasets: [{
          backgroundColor: ["#DB3F29", "#D7A449"],
          data: [this.nOfDeathsPredicted,  this.nToPredict]
        }]},
    });
  }
